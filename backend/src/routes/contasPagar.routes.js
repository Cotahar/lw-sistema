...
router.get('/', requerAcessoModulo('contas_pagar', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.get('/:id', requerAcessoModulo('contas_pagar', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.post('/', requerAcessoModulo('contas_pagar', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const info = db.prepare(`
    INSERT INTO contas_pagar (fornecedor_id, centro_custo_id, descricao, valor, data_vencimento, status, origem_tipo)
    VALUES (?, ?, ?, ?, ?, 'Pendente', 'Outro')
  `).run(fornecedor_id || null, centro_custo_id || null, descricao, valor, data_vencimento);
  const conta = db.prepare('SELECT * FROM contas_pagar WHERE id = ?').get(info.lastInsertRowid);
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'contas_pagar', registroId: conta.id, acao: 'INSERT', depois: conta });
  res.status(201).json(conta);
}));

router.put('/:id', requerAcessoModulo('contas_pagar', 'Gerenciar'), asyncHandler(async (req, res) => {
  const antes = db.prepare('SELECT * FROM contas_pagar WHERE id = ?').get(req.params.id);
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'contas_pagar', registroId: depois.id, acao: 'UPDATE', antes, depois });
  res.json(depois);
}));
...
router.post('/:id/baixar', requerAcessoModulo('contas_pagar', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const resultado = withTransaction(db, () => {
    const contaPagar = db.prepare('SELECT * FROM contas_pagar WHERE id = ?').get(req.params.id);
    ...
    `).run(conta_bancaria_id, valorBaixa, data_pagamento || null, contaPagar.descricao, contaPagar.id, req.usuario.id);
...
router.delete('/:id', requerAcessoModulo('contas_pagar', 'Gerenciar'), asyncHandler(async (req, res) => {
  const antes = db.prepare('SELECT * FROM contas_pagar WHERE id = ?').get(req.params.id);
  if (!antes) throw new ApiError(404, 'Conta a pagar nao encontrada.');
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'contas_pagar', registroId: antes.id, acao: 'DELETE', antes });
  res.status(204).send();
}));
