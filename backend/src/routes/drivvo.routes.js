...
function buscarVeiculoPorNome(nomeVeiculo, mapaPlacasPorNome) {
  ...
  const todos = db.prepare('SELECT * FROM veiculos').all();
...
function lancarDespesaViagem(viagem, categoria, valorCentavos, dataIso, extras = {}, usuarioId = null) {
  ...
  const info = db.prepare(`
    INSERT INTO despesas_viagem (viagem_id, centro_custo_id, categoria_id, valor, data, pago_por, posto_fornecedor_id, litragem, preco_litro, descricao, criado_por)
    VALUES (?, ?, ?, ?, ?, 'Empresa', ?, ?, ?, ?, ?)
  `).run(viagem.id, centroCusto.id, categoria.id, valorCentavos, dataIso, extras.postoFornecedorId || null, extras.litragem || null, extras.precoLitro || null, extras.descricao || null, usuarioId);
  ...
  `).run(extras.postoFornecedorId || null, `${categoria.nome} - viagem #${viagem.id} (Drivvo)`, valorCentavos, dataIso, despesa.id);

  return despesa;
}

function jaImportado(chave) {
  return Boolean(db.prepare('SELECT 1 FROM importacoes_drivvo WHERE chave_externa = ?').get(chave));
}

function registrarPendencia({ chave, secao, dadosBrutos, motivo }) {
  db.prepare(`
    INSERT INTO importacoes_drivvo (chave_externa, secao, status, dados_brutos, motivo_pendencia)
    VALUES (?, ?, 'PendenteRevisao', ?, ?)
  `).run(chave, secao, JSON.stringify(dadosBrutos), motivo);
}

function registrarImportado({ chave, secao, entidadeTipo, entidadeId, dadosBrutos, usuarioId }) {
  db.prepare(`
    INSERT INTO importacoes_drivvo (chave_externa, secao, status, entidade_tipo, entidade_id, dados_brutos, resolvido_em, resolvido_por)
    VALUES (?, ?, 'Importado', ?, ?, ?, datetime('now'), ?)
  `).run(chave, secao, entidadeTipo, entidadeId, JSON.stringify(dadosBrutos), usuarioId);
}
...
router.post('/importar', requerAcessoModulo('viagens', 'Gerenciar'), upload.single('arquivo'), asyncHandler(async (req, res) => {
  ...
  withTransaction(db, () => {
    ...
    for (const linha of dados.abastecimentos) {
      ...
      if (jaImportado(chave)) { resumo.abastecimento.jaExistiam++; continue; }
      if (!dataIso) { registrarPendencia({ chave, secao: 'Abastecimento', dadosBrutos: linha, motivo: 'Data invalida no arquivo.' }); resumo.abastecimento.pendentes++; continue; }

      const veiculo = buscarVeiculoPorNome(linha.nome_veiculo, mapaPlacasPorNome);
      if (!veiculo) { registrarPendencia({ chave, secao: 'Abastecimento', dadosBrutos: linha, motivo: `Veiculo "${linha.nome_veiculo}" nao encontrado no Frotista.` }); resumo.abastecimento.pendentes++; continue; }

      const candidatas = buscarViagensCandidatas(veiculo.id, dataIso);
      ...
      registrarImportado({ chave, secao: 'Abastecimento', entidadeTipo: 'DespesaViagem', entidadeId: despesa.id, dadosBrutos: linha, usuarioId: req.usuario.id });
      ...
      if (linha.combustivel_2 && paraCentavos(linha.valor_total_2) > 0) {
        ...
        if (!jaImportado(chaveArla)) {
          ...
          registrarImportado({ chave: chaveArla, secao: 'Abastecimento', entidadeTipo: 'DespesaViagem', entidadeId: despesaArla.id, dadosBrutos: linha, usuarioId: req.usuario.id });
        }
      }
    }
    ...
    for (const linha of dados.despesas) {
      ...
      if (jaImportado(chave)) { resumo.despesa.jaExistiam++; continue; }
      if (!dataIso) { registrarPendencia({ chave, secao: 'Despesa', dadosBrutos: linha, motivo: 'Data invalida no arquivo.' }); resumo.despesa.pendentes++; continue; }

      const ehAdiantamento = /adiantamento/i.test(linha['tipo de despesa'] || '');
      const veiculo = buscarVeiculoPorNome(linha['nome do veiculo'], mapaPlacasPorNome);
      ...
      if (!veiculo) { registrarPendencia({ chave, secao: 'Despesa', dadosBrutos: linha, motivo: `Veiculo "${linha['nome do veiculo']}" nao encontrado no Frotista.` }); resumo.despesa.pendentes++; continue; }
      const candidatas = buscarViagensCandidatas(veiculo.id, dataIso);
      if (candidatas.length !== 1) {
        registrarPendencia({
          chave, secao: 'Despesa', dadosBrutos: { ...linha, veiculo_id: veiculo.id },
      ...
      const viagem = candidatas[0];
      const info = db.prepare(`
        INSERT INTO viagem_adiantamentos (viagem_id, valor, data, descricao, criado_por)
        VALUES (?, ?, ?, ?, ?)
      `).run(viagem.id, paraCentavos(linha['valor total']), dataIso, linha.observacao || 'Importado do Drivvo', req.usuario.id);
      registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'viagem_adiantamentos', registroId: info.lastInsertRowid, acao: 'INSERT', depois: { viagem_id: viagem.id } });
      registrarImportado({ chave, secao: 'Despesa', entidadeTipo: 'ViagemAdiantamento', entidadeId: info.lastInsertRowid, dadosBrutos: linha, usuarioId: req.usuario.id });
    ...
    for (const linha of dados.receitas) {
      ...
      if (jaImportado(chave)) { resumo.receita.jaExistiam++; continue; }
      const veiculo = buscarVeiculoPorNome(linha['nome do veiculo'], mapaPlacasPorNome);
      registrarPendencia({
...
router.get('/pendencias', requerAcessoModulo('viagens', 'Visualizar'), asyncHandler(async (req, res) => {
  ...
  const linhas = db.prepare('SELECT * FROM importacoes_drivvo WHERE status = ? ORDER BY id DESC').all(status || 'PendenteRevisao');
...
router.post('/pendencias/:id/resolver', requerAcessoModulo('viagens', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const pendencia = db.prepare("SELECT * FROM importacoes_drivvo WHERE id = ? AND status = 'PendenteRevisao'").get(req.params.id);
