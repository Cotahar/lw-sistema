...
router.get('/', requerAcessoModulo('contas_bancarias', 'Visualizar'), asyncHandler(async (req, res) => {
  res.json(db.prepare('SELECT * FROM contas_bancarias ORDER BY nome').all());
}));

router.get('/:id', requerAcessoModulo('contas_bancarias', 'Visualizar'), asyncHandler(async (req, res) => {
  const conta = db.prepare('SELECT * FROM contas_bancarias WHERE id = ?').get(req.params.id);
  if (!conta) throw new ApiError(404, 'Conta bancaria nao encontrada.');
  res.json(conta);
}));

router.post('/', requerAcessoModulo('contas_bancarias', 'Gerenciar'), asyncHandler(async (req, res) => {
  const { nome, banco, agencia, conta, saldo_atual } = req.body;
  if (!nome) throw new ApiError(400, 'Informe o nome da conta.');
  const info = db.prepare('INSERT INTO contas_bancarias (nome, banco, agencia, conta, saldo_atual) VALUES (?, ?, ?, ?, ?)')
    .run(nome, banco || null, agencia || null, conta || null, saldo_atual || 0);
  const nova = db.prepare('SELECT * FROM contas_bancarias WHERE id = ?').get(info.lastInsertRowid);
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'contas_bancarias', registroId: nova.id, acao: 'INSERT', depois: nova });
  res.status(201).json(nova);
}));

router.put('/:id', requerAcessoModulo('contas_bancarias', 'Gerenciar'), asyncHandler(async (req, res) => {
  const antes = db.prepare('SELECT * FROM contas_bancarias WHERE id = ?').get(req.params.id);
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'contas_bancarias', registroId: depois.id, acao: 'UPDATE', antes, depois });
  res.json(depois);
}));

router.get('/:id/movimentacoes', requerAcessoModulo('contas_bancarias', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.post('/:id/movimentacoes', requerAcessoModulo('contas_bancarias', 'Gerenciar'), asyncHandler(async (req, res) => {
  const conta = db.prepare('SELECT * FROM contas_bancarias WHERE id = ?').get(req.params.id);
  if (!conta) throw new ApiError(404, 'Conta bancaria nao encontrada.');
  ...
  const movimentacao = withTransaction(db, () => {
    const info = db.prepare(`
      INSERT INTO movimentacoes_caixa (conta_bancaria_id, tipo, valor, data, descricao, origem_tipo, criado_por)
      VALUES (?, ?, ?, COALESCE(?, date('now')), ?, 'Ajuste', ?)
    `).run(conta.id, tipo, valor, data || null, descricao || null, req.usuario.id);
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'movimentacoes_caixa', registroId: movimentacao.id, acao: 'INSERT', depois: movimentacao });
