...
router.get('/itens', requerAcessoModulo('estoque', 'Visualizar'), asyncHandler(async (req, res) => {
  const { search } = req.query;
  if (search) {
    return res.json(db.prepare('SELECT * FROM estoque_itens WHERE nome LIKE ? ORDER BY nome').all(`%${search}%`));
  }
  res.json(db.prepare('SELECT * FROM estoque_itens ORDER BY nome').all());
}));

router.get('/itens/:id', requerAcessoModulo('estoque', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.post('/itens', requerAcessoModulo('estoque', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const info = db.prepare(`
    INSERT INTO estoque_itens (nome, categoria, unidade_medida, estoque_minimo)
    VALUES (?, ?, ?, ?)
  `).run(nome, categoria, unidade_medida || 'UN', estoque_minimo || 0);
  const item = db.prepare('SELECT * FROM estoque_itens WHERE id = ?').get(info.lastInsertRowid);
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'estoque_itens', registroId: item.id, acao: 'INSERT', depois: item });
  res.status(201).json(item);
}));

router.put('/itens/:id', requerAcessoModulo('estoque', 'Gerenciar'), asyncHandler(async (req, res) => {
  const antes = db.prepare('SELECT * FROM estoque_itens WHERE id = ?').get(req.params.id);
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'estoque_itens', registroId: depois.id, acao: 'UPDATE', antes, depois });
  res.json(depois);
}));

router.delete('/itens/:id', requerAcessoModulo('estoque', 'Gerenciar'), asyncHandler(async (req, res) => {
  const antes = db.prepare('SELECT * FROM estoque_itens WHERE id = ?').get(req.params.id);
  if (!antes) throw new ApiError(404, 'Item de estoque nao encontrado.');
  db.prepare('DELETE FROM estoque_itens WHERE id = ?').run(req.params.id);
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'estoque_itens', registroId: antes.id, acao: 'DELETE', antes });
  res.status(204).send();
}));
...
router.get('/movimentacoes', requerAcessoModulo('estoque', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.post('/movimentacoes', requerAcessoModulo('estoque', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const resultado = withTransaction(db, () => {
    ...
    const info = db.prepare(`
      INSERT INTO estoque_movimentacoes (item_id, tipo, quantidade, custo_unitario, fornecedor_id, veiculo_destino_id, os_id, observacao, criado_por)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(item_id, tipo, quantidade, custo_unitario, fornecedor_id || null, veiculo_destino_id || null, os_id || null, observacao || null, req.usuario.id);
    ...
    if (tipo === 'Entrada') {
      ...
      `).run(fornecedor_id || null, `Compra de estoque: ${item.nome} x${quantidade}`, Math.round(quantidade * custo_unitario), info.lastInsertRowid);
    } else {
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'estoque_movimentacoes', registroId: resultado.movimentacao.id, acao: 'INSERT', depois: resultado.movimentacao });
