...
function validarCarretaPadrao(tipo, carretaPadraoId) {
  ...
  const carreta = db.prepare('SELECT * FROM veiculos WHERE id = ?').get(carretaPadraoId);
...
router.get('/', requerAcessoModulo('veiculos', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.get('/:id', requerAcessoModulo('veiculos', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.post('/', requerAcessoModulo('veiculos', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  validarCarretaPadrao(tipo, carreta_padrao_id);

  const veiculo = withTransaction(db, () => {
    const info = db.prepare(`
      INSERT INTO veiculos (placa, tipo, qtd_eixos, marca, modelo, ano_fabricacao, carreta_padrao_id, hodometro_atual)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(placa.toUpperCase(), tipo, qtd_eixos, marca || null, modelo || null, ano_fabricacao || null, carreta_padrao_id || null, hodometro_atual || 0);
    // Todo veiculo e, por si so, um centro de custo (usado por despesas, OS, DRE...).
    db.prepare('INSERT INTO centros_custo (tipo, veiculo_id, nome) VALUES (?, ?, ?)')
      .run('Veiculo', info.lastInsertRowid, placa.toUpperCase());
    return db.prepare('SELECT * FROM veiculos WHERE id = ?').get(info.lastInsertRowid);
  });
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'veiculos', registroId: veiculo.id, acao: 'INSERT', depois: veiculo });
  res.status(201).json(veiculo);
}));

router.put('/:id', requerAcessoModulo('veiculos', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  validarCarretaPadrao(tipo, carretaPadraoId);
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'veiculos', registroId: depois.id, acao: 'UPDATE', antes, depois });
  res.json(depois);
}));

router.delete('/:id', requerAcessoModulo('veiculos', 'Gerenciar'), asyncHandler(async (req, res) => {
  const antes = db.prepare('SELECT * FROM veiculos WHERE id = ?').get(req.params.id);
  if (!antes) throw new ApiError(404, 'Veiculo nao encontrado.');
  db.prepare('DELETE FROM veiculos WHERE id = ?').run(req.params.id);
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'veiculos', registroId: antes.id, acao: 'DELETE', antes });
  res.status(204).send();
}));

// Historico de hodometro e ajuste manual (fallback quando a telemetria Onixsat falha).
router.get('/:id/hodometro', requerAcessoModulo('veiculos', 'Visualizar'), asyncHandler(async (req, res) => {
  const eventos = db.prepare('SELECT * FROM hodometro_eventos WHERE veiculo_id = ? ORDER BY data_hora DESC').all(req.params.id);
  res.json(eventos);
}));

router.post('/:id/hodometro', requerAcessoModulo('veiculos', 'Gerenciar'), asyncHandler(async (req, res) => {
  const veiculo = db.prepare('SELECT * FROM veiculos WHERE id = ?').get(req.params.id);
  if (!veiculo) throw new ApiError(404, 'Veiculo nao encontrado.');
  ...
  const evento = withTransaction(db, () => {
    const info = db.prepare(`
      ...
      VALUES (?, ?, 'Manual', ?, ?)
    `).run(veiculo.id, km, req.usuario.id, observacao || null);
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'veiculos', registroId: veiculo.id, acao: 'UPDATE', antes: { hodometro_atual: veiculo.hodometro_atual }, depois: { hodometro_atual: km } });
