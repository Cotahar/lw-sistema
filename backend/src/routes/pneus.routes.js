...
function registrarEvento({ pneuId, tipoEvento, veiculoId, eixo, lado, kmVeiculo, fornecedorId, custo, observacao, usuarioId }) {
  const info = db.prepare(`
    INSERT INTO pneu_eventos (pneu_id, tipo_evento, veiculo_id, eixo, lado, km_veiculo, fornecedor_id, custo, observacao, criado_por)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(pneuId, tipoEvento, veiculoId || null, eixo || null, lado || null, kmVeiculo || null, fornecedorId || null, custo ?? null, observacao || null, usuarioId);
  return db.prepare('SELECT * FROM pneu_eventos WHERE id = ?').get(info.lastInsertRowid);
}

router.get('/', requerAcessoModulo('pneus', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.get('/:id', requerAcessoModulo('pneus', 'Visualizar'), asyncHandler(async (req, res) => {
  const pneu = db.prepare('SELECT * FROM pneus WHERE id = ?').get(req.params.id);
  if (!pneu) throw new ApiError(404, 'Pneu nao encontrado.');
  res.json(pneu);
}));

router.get('/:id/eventos', requerAcessoModulo('pneus', 'Visualizar'), asyncHandler(async (req, res) => {
...
router.post('/', requerAcessoModulo('pneus', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const pneu = withTransaction(db, () => {
    ...
    const evento = registrarEvento({
      pneuId: novoPneu.id, tipoEvento: 'Aquisicao', fornecedorId: fornecedor_id, custo: custo_unitario, usuarioId: req.usuario.id,
    });
    ...
    `).run(fornecedor_id || null, `Compra de pneu: ${novoPneu.numero_fogo}`, custo_unitario, evento.id);

    return novoPneu;
  });

  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'pneus', registroId: pneu.id, acao: 'INSERT', depois: pneu });
  res.status(201).json(pneu);
}));

router.put('/:id', requerAcessoModulo('pneus', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  registrarAuditoria({ usuarioId: req.usuario.id, tabela: 'pneus', registroId: depois.id, acao: 'UPDATE', antes, depois });
  res.json(depois);
}));

router.post('/:id/instalar', requerAcessoModulo('pneus', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const pneu = withTransaction(db, () => {
    ...
    registrarEvento({
      pneuId: atual.id, tipoEvento: 'Instalacao', veiculoId: veiculo_id, eixo, lado, kmVeiculo: km_veiculo,
...
router.post('/:id/enviar-recapagem', requerAcessoModulo('pneus', 'Gerenciar'), asyncHandler(async (req, res) => {
...
router.post('/:id/retornar-recapagem', requerAcessoModulo('pneus', 'Gerenciar'), asyncHandler(async (req, res) => {
  ...
  const pneu = withTransaction(db, () => {
    ...
    db.prepare(`
      INSERT INTO contas_pagar (fornecedor_id, descricao, valor, data_vencimento, status, origem_tipo, origem_id)
      VALUES (?, ?, ?, date('now'), 'Pendente', 'PneuEvento', ?)
    `).run(fornecedor_id || null, `Recapagem do pneu: ${atual.numero_fogo}`, custo, evento.id);
